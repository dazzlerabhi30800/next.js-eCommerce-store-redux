...
const SideBar = () => {
  ...
  return (
    <aside
      ...
      } absolute top-0 left-0 flex flex-col overflow-y-auto bg-pink-200 h-screen w-[70%] min-w-[150px] md:max-w-[400px] shadow-xl transitionL`}
    >
      {categories?.map((category: category, index: number) => (
        <button
          onClick={() => fetchNewProducts(category.slug)}
          className="py-4 px-6 text-black text-sm sm:text-base transitionL hover:bg-black hover:text-white"
          key={index}
        >
          {category.name}
        </button>
